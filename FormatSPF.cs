...
namespace IfcDoc.Format.SPF
{
    ...
    {        
        ...
        private object ParseValue(Type type, string strval)
        {
            ...
            // value types
            Type typewrap = null;
            while (type.IsValueType && !type.IsPrimitive && !type.IsEnum)
            {
        ...
        private System.Collections.IList ParseList(Type t, string line)
        {
            ...
            while (x < line.Length - 1)
            {
                ...
                if (bValue)
                {
                    if (m_markup != null && this.m_parsescope == ParseScope.DataFields && nExpectedCount > 0)
                    {
                        m_markup.Append(",");
                    }
        ...
        public void InitHeaders(string filename, string schema)
        {
            ...
            hName.PreprocessorVersion = "buildingSMART IFCDOC 11.6"; // was "buildingSMART IFCDOC" for 2.7 and earlier;
