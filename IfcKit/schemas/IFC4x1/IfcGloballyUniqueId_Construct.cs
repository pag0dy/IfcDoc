...
namespace BuildingSmart.IFC.IfcUtilityResource
{
    public partial struct IfcGloballyUniqueId
    {
        ...
        public static IfcGloballyUniqueId New()
        {
            Guid guid = Guid.NewGuid();
            string format = Format(guid);
            IfcGloballyUniqueId ifcguid = new IfcGloballyUniqueId(format);
            return ifcguid;
        }

        private static Guid Parse(string format64)
        {
            int i, j, m;
            uint[] num = new uint[6];
